...
public interface OsmTagInfoRestClient {
   ...
   */
  @GET("keys/all?qtype=key&sortname=count_ways&sortorder=desc&filter=characters_plain") SuggestionsDto getSuggestions(@Query("query") String query,
      @Query("page") Integer pageNumber, @Query("rp") Integer resultPerPage);
   ...
   */
  @GET("key/wiki_pages") Wiki getWikiPages(@Query("key") String key);
   ...
   */
  @GET("key/combinations?sortname=to_count&sortorder=desc&qtype=other_key&filter=all") CombinationsDto getCombinations(@Query("key") String key,
      @Query("page") Integer pageNumber, @Query("rp") Integer resultPerPage);
}
